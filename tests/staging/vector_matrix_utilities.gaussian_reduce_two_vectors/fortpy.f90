...
module fortpy
  implicit none
  private
  public pysave, dp, sp, si, li, fpy_linevalue_count, fpy_newunit
...
contains  
 ...
 subroutine random_integer_2d(variable, min, max)
  ...
  subroutine file_open(filename, template_name)
    ...
    if (exist) then
       open(fpy_newunit(fileunit), file="test.txt", status="old", position="append",  &
            action="write", iostat=ioerr)
    else
       open(fpy_newunit(fileunit), file="test.txt", status="new", action="write", iostat=ioerr)
    end if
