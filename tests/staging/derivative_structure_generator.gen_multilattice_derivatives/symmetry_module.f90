...
integer, pointer:: atomType(:)         ! intent(inout): Atom types represented as integers
real(dp), pointer:: atom_pos(:,:)      ! intent(inout): Positions of the basis atoms
logical, intent(in):: lattCoords       ! .true. if positions are in lattice coordinates
real(dp), intent(in), optional   :: eps_        ! eps
integer, pointer, optional       :: removed_(:) ! the indices of the atoms that have been removed
